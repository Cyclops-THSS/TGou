...
class ShopCategory (models.Model):
	...
	#parent = models.ForeignKey(ShopCategory, on_delete=models.CASCADE)	#	父类别标识符
...
class Cart (models.Model):
	consumer = models.ForeignKey(Consumer, on_delete=models.CASCADE)	#	所属消费者标识符
...
class Comment (models.Model):
	commodity = models.ForeignKey(Commodity, on_delete=models.CASCADE)	#	被评论商品标识符
	grade = models.DecimalField(max_digits=3, decimal_places=2)	#	评分
	message = models.TextField()	#	评论内容
	time = models.DateTimeField()	#	创建时间

class Grading (models.Model):
	consumer = models.ForeignKey(Consumer, on_delete=models.CASCADE)	#	关联消费者标识符
